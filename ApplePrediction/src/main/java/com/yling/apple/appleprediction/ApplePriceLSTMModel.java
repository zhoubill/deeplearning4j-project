...
public class ApplePriceLSTMModel {
...
//				.tBPTTForwardLength(60)
	...
	public static void train(MultiLayerNetwork net, ApplePriceDataIterator iterator) throws IOException {
		//这里执行迭代训练
	...
	private static INDArray getInitArray(ApplePriceDataIterator iter) {
		double[] maxNums = iter.getMaxArr();
		INDArray initArray = Nd4j.zeros(1, 3, 1);
		initArray.putScalar(new int[] { 0, 0, 0 }, 800 / maxNums[0]);
		initArray.putScalar(new int[] { 0, 1, 0 }, 900 / maxNums[1]);
		initArray.putScalar(new int[] { 0, 2, 0 }, 700 / maxNums[2]);
		return initArray;
	}
...
//		System.loadLibrary("mkl_rt");
		String inputFile = ApplePriceLSTMModel.class.getClassLoader().getResource("fruitApplePrice.txt").getPath();
		int batchSize = 1;
		int exampleLength = 30;
		//初始化深度神经网络
		ApplePriceDataIterator iterator = new ApplePriceDataIterator();
